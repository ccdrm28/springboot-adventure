package com.example.springbootadventure.config;
...
import com.example.springbootadventure.constant.ResponseStatus;
...
public class SwaggerConfig {
  ...
  public Docket openApi() {
    return new Docket(DocumentationType.SWAGGER_2).groupName("Test Group").apiInfo(apiInfo())
        .select().apis(RequestHandlerSelectors.withMethodAnnotation(ApiOperation.class))
        .paths(PathSelectors.any()).build().globalOperationParameters(getGlobalRequestParameters())
  ...
  private List<ResponseMessage> getGlobalResponse() {
    return ResponseStatus.HTTP_STATUS_ALL.stream().map(
        a -> new ResponseMessageBuilder().message(a.getDescription())
            .code(Integer.parseInt(a.getResponseCode())).build()).collect(Collectors.toList());
  }
  ...
  private List<Parameter> getGlobalRequestParameters() {
    ...
    parameters.add(
        new ParameterBuilder().name("AppKey").description("App Key").defaultValue("app-key-xxx-1")
  ...
  private ApiInfo apiInfo() {
    return new ApiInfoBuilder().title("Swagger API").description("test api")
        ...
        .termsOfServiceUrl("http://xxxxxx.com/").version("1.0").build();
  }
}
